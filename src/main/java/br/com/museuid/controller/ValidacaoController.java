...
public class ValidacaoController extends AnchorPane {
    ...
    public ValidacaoController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/validacao.fxml"));
    ...
    void finalizar(ActionEvent event) {
        ...
        if (resposta == Dialogo.Resposta.YES) {
            if (statusValidacao) {
                ...
                Validacao id = validacaoDAO.validacao(validacao);

                for (ValidacaoItem item : validacao.getItens()) {
                    validacaoDAO.item(id.getId(), item);
                }
    ...
    private void baseDados() {
        ...
        this.catalogacoes = catalogacaoDAO.listar();
    }
