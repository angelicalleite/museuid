...
import br.com.museuid.banco.dao.LocalDAO;
import br.com.museuid.banco.dao.SetorDAO;
...
public class LocalController extends AnchorPane {
    ...
    public LocalController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/local.fxml"));
    ...
    void salvar(ActionEvent event) {
        ...
        if (vazio) {
            ...
            if (idLocal == 0) {
                new LocalDAO().inserir(local);
                Mensagem.info("Local cadastrada com sucesso!");
            } else {
                new LocalDAO().editar(local);
    ...
    void excluir(ActionEvent event) {
        try {
            ...
            if (response == Dialogo.Resposta.YES) {
                new LocalDAO().excluir(local.getId());
                sincronizarBase();
                tabela();
            }
     ...
     */
    public void combos() {
        Combo.popular(cbSetor, new SetorDAO().combo());
    }
    ...
    private void sincronizarBase() {
        listaLocal = new LocalDAO().listar();
    }
