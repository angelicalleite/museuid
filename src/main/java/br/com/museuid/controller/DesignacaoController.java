...
public class DesignacaoController extends AnchorPane {
    ...
    public DesignacaoController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/designacao.fxml"));
    ...
    void salvar(ActionEvent event) {
        ...
        if (vazio) {
        ...
        } else if (designacaoDAO.isDesignacao(genero, idDesignacao)) {
            ...
            if (idDesignacao == 0) {
                designacaoDAO.inserir(designacao);
                //Mensagem.info("Designação cadastrada com sucesso!");
            } else {
                designacaoDAO.editar(designacao);
    ...
    void excluir(ActionEvent event) {
        try {
            ...
            if (response == Dialogo.Resposta.YES) {
                new DesignacaoDAO().excluir(designacao.getId());
                sincronizarBase();
                tabela();
            }
    ...
    private void sincronizarBase() {
        listaDesignacao = new DesignacaoDAO().listar();
    }
