...
import br.com.museuid.banco.dao.OrganizacaoDAO;
...
public class OrganizacaoController extends AnchorPane {
    ...
    public OrganizacaoController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/organizacao.fxml"));
    ...
    void salvar(ActionEvent event) {
        ...
        if (vazio) {
            ...
            if (idOrganizacao == 0) {
                organizacaoDAO.inserir(orgao);
                Mensagem.info("Organização cadastrada com sucesso!");
            } else {
                organizacaoDAO.editar(orgao);
    ...
    void excluir(ActionEvent event) {
        try {
            ...
            if (response == Dialogo.Resposta.YES) {
                new OrganizacaoDAO().excluir(orgao.getId());
                sincronizarBase();
                tabela();
            }
    ...
    private void sincronizarBase() {
        listaOrganizacao = new OrganizacaoDAO().listar();
    }
