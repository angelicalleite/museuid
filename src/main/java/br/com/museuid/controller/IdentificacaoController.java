...
import br.com.museuid.banco.dao.IdentificacaoDAO;
...
public class IdentificacaoController extends AnchorPane {
    ...
    public IdentificacaoController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("../../../../../resources/fxml/identificacao.fxml"));
    ...
    public void identificar(String identificador) {
        ...
        } else if (!new IdentificacaoDAO().validarIdentificador(identificador)) {
            Mensagem.alerta("Fóssil não encontrado!");
        } else {
            fossil = new IdentificacaoDAO().identificar(identificador);
            info(fossil);
        }
    ...
    private void info(Catalogacao fossil) {
        ...
        String emprestimo = new CatalogacaoDAO().infoNumEmprestimo(fossil.getId());
