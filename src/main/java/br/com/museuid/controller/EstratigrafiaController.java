...
public class EstratigrafiaController extends AnchorPane {
    ...
    public EstratigrafiaController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/estratigrafia.fxml"));
    ...
    void salvar(ActionEvent event) {
        ...
        if (vazio) {
        ...
        } else if (new EstratigrafiaDAO().isEstratigrafia(formacao, idEstratigrafia)) {
            ...
            if (idEstratigrafia == 0) {
                new EstratigrafiaDAO().inserir(estratigrafia);
                Mensagem.info("Estratigrafia cadastrada com sucesso!");
            } else {
                new EstratigrafiaDAO().editar(estratigrafia);
    ...
    void excluir(ActionEvent event) {
        try {
            ...
            if (response == Dialogo.Resposta.YES) {
                new EstratigrafiaDAO().excluir(estratigrafia.getId());
                sincronizarBase();
                tabela();
            }
    ...
    private void sincronizarBase() {
        listaEstratigrafia = new EstratigrafiaDAO().listar();
    }
