...
import br.com.museuid.banco.dao.ColecaoDAO;
import br.com.museuid.banco.dao.DesignacaoDAO;
import br.com.museuid.banco.dao.EstratigrafiaDAO;
...
public class CatalogarController extends AnchorPane {
    ...
    public CatalogarController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/catalogar.fxml"));
    ...
    void salvar(ActionEvent event) {
        ...
        if (vazio) {
        ...
        } else if (new CatalogacaoDAO().validarNumeroOrdem(numOrdem, idCatalogacao)) {
            Nota.alerta("Ordem já cadastrada!");
        } else if (new CatalogacaoDAO().validarEtiquetaRFID(etiquetaRfid, idCatalogacao)) {
            ...
            if (idCatalogacao == 0) {
                new CatalogacaoDAO().inserir(catalogacao);
                Mensagem.info("Catalogação cadastrada com sucesso!");
            } else {
                new CatalogacaoDAO().editar(catalogacao);
    ...
    void excluir(ActionEvent event) {
        try {
            ...
            if (response == Dialogo.Resposta.YES) {
                new CatalogacaoDAO().excluir(catalogacao.getId());
                sincronizarBase();
                tabela();
            }
    ...
    public void sincronizarBase() {
        listaCatalogacao = new CatalogacaoDAO().listar();
    }
     ...
     */
    public void combo() {
        Combo.popular(cbColecao, new ColecaoDAO().combo());
        Combo.popular(cbEstratigrafia, new EstratigrafiaDAO().combo());
        Combo.popular(cbDesignacao, new DesignacaoDAO().combo());
    }
