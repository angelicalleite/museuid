...
import br.com.museuid.banco.dao.MovimentacaoDAO;
...
public class MovimentacaoController extends AnchorPane {
    ...
    public MovimentacaoController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/movimentacao.fxml"));
    ...
    void salvar(ActionEvent event) {
        ...
        if (vazio) {
            ...
            if (idMovimentacao == 0) {
                movimentacaoDAO.inserir(movi);
                Mensagem.info("Movimentação cadastrada com sucesso!");
            } else {
                movimentacaoDAO.editar(movi);
    ...
    void excluir(ActionEvent event) {
        try {
            ...
            if (response == Dialogo.Resposta.YES) {
                movimentacaoDAO.excluir(movimentacao.getId());
                sincronizarBase();
                tabela();
            }
    ...
    private void sincronizarBase() {
        MovimentacaoDAO movimentacaoDAO = new MovimentacaoDAO();
        listaMovimentacao = movimentacaoDAO.listar();
    }
