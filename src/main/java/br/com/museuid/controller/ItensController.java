...
import br.com.museuid.banco.dao.CatalogacaoDAO;
import br.com.museuid.banco.dao.EmprestimoDAO;
...
public class ItensController extends AnchorPane {
    ...
    public ItensController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("../../../../../resources/fxml/itens.fxml"));
    ...
    void adicionar(ActionEvent event) {
        ...
        if (vazio) {
            ...
            int catalogacao = new CatalogacaoDAO().infoId(ordem);//caso numero de ordem não seja encontrado retornar 0

            if (catalogacao == 0) {
                Nota.alerta("Catalogação não encontrada!");
            } else if (new CatalogacaoDAO().isEmprestada(catalogacao)) {
                ...
                new EmprestimoDAO().addItem(item);//adiciona item ao emprestimo
                new CatalogacaoDAO().statusEmprestada(catalogacao, true);//atualiza status de emprestimo da catalogacao para ativo = 1
            }
    ...
    void remover(ActionEvent event) {
        try {
            ...
            new EmprestimoDAO().excluirItem(item.getId());//adiciona item ao emprestimo
            new CatalogacaoDAO().statusEmprestada(catalogacao, false);//desativar status emprestimo catalogacao
     ...
     */
    public void combos() {
        Combo.popular(cbEmprestimo, new EmprestimoDAO().comboItens());
    }
    ...
    private void sincronizarBase(int emprestimo) {
        listaEmprestimoItens = new EmprestimoDAO().listarItens(emprestimo);
    }
