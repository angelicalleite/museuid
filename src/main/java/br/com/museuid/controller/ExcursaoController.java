...
import br.com.museuid.banco.dao.ExcursaoDAO;
...
public class ExcursaoController extends AnchorPane {
    ...
    public ExcursaoController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/excursao.fxml"));
    ...
    void salvar(ActionEvent event) {
        ...
        if (vazio) {
            ...
            if (idExcursao == 0) {
                new ExcursaoDAO().inserir(excursao);
                Mensagem.info("Excursão cadastrado com sucesso!");
            } else {
                new ExcursaoDAO().editar(excursao);
    ...
    void excluir(ActionEvent event) {
        try {
            ...
            if (response == Dialogo.Resposta.YES) {
                new ExcursaoDAO().excluir(excursao.getId());
                sincronizarBase();
                tabela();
            }
    ...
    private void sincronizarBase() {
        listaExcursao = new ExcursaoDAO().listar();
    }
