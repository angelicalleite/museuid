...
import br.com.museuid.banco.dao.VisitanteDAO;
...
public class VisitanteController extends AnchorPane {
    ...
    public VisitanteController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/visitante.fxml"));
    ...
    void salvar(ActionEvent event) {
        ...
        if (vazio) {
            ...
            if (idVisitante == 0) {
                new VisitanteDAO().inserir(visitante);
                Mensagem.info("Visitante cadastrado com sucesso!");
            } else {
                new VisitanteDAO().editar(visitante);
    ...
    void excluir(ActionEvent event) {
        try {
            ...
            if (response == Dialogo.Resposta.YES) {
                new VisitanteDAO().excluir(visitante.getId());
                sincronizarBase();
                tabela();
            }
    ...
    private void sincronizarBase() {
        listaVisitante = new VisitanteDAO().listar();
    }
