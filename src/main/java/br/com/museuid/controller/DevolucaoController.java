...
public class DevolucaoController extends AnchorPane {
    ...
    public DevolucaoController() {
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/devolucao.fxml"));
    ...
    void devolucao(ActionEvent event) {
        ...
        if (vazio) {
            ...
            List<Catalogacao> itens = emprestimoDAO.itensEmprestimo(emprestimo.getId());

            if (itens.isEmpty()) {
                Nota.alerta("Empréstimo não contém itens para devolução!");
            } else {
                emprestimoDAO.devolucao(emprestimo.getId(), dataEntrega, observacao);//relizar devolução item

                for (Catalogacao catalogacao : itens) {//mudar status de itens para inativo
                    new CatalogacaoDAO().statusEmprestada(catalogacao.getId(), false);
                }
     ...
     */
    public void combos() {
        Combo.popular(cbEmprestimo, new EmprestimoDAO().comboDevolucao());
    }
